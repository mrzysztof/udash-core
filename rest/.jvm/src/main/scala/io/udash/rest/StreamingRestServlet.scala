...
object StreamingRestServlet {
   ...
   */
  @explicitGenerics def apply[RestApi: StreamingRawRest.AsRawRpc : RestMetadata](
    apiImpl: RestApi,
    handleTimeout: FiniteDuration = DefaultHandleTimeout,
    maxPayloadSize: Long = DefaultMaxPayloadSize
  )(implicit
    scheduler: Scheduler
  ): StreamingRestServlet = new StreamingRestServlet(StreamingRawRest.asHandleRequest[RestApi](apiImpl), handleTimeout, maxPayloadSize)

  private final val BufferSize = 8192
}

class StreamingRestServlet(
  handleRequest: StreamingRawRest.HandleRequest,
  handleTimeout: FiniteDuration = DefaultHandleTimeout,
  maxPayloadSize: Long = DefaultMaxPayloadSize
)(implicit
  scheduler: Scheduler
  ...
  import StreamingRestServlet.*

  override def service(request: HttpServletRequest, response: HttpServletResponse): Unit = {
    val asyncContext = request.startAsync()
    val udashRequest = Try(readRequest(request))

    val cancelable = handleRequest(udashRequest.get)
      .doOnStart(rr => Task(writeHeaders(response, rr)))
      .doOnNext(rr => Task(writeResponse(response, rr)))
      .doOnComplete(Task(asyncContext.complete()))
      .doOnError(ex => Task {
        ex match {
          case e: HttpErrorException => writeResponse(response, e.toResponse)
          case e: Throwable =>
    ...
    asyncContext.addListener(new AsyncListener {
      def onComplete(event: AsyncEvent): Unit = cancelable.cancel()
      def onTimeout(event: AsyncEvent): Unit = {
        cancelable.cancel()
        writeFailure(response, Opt(s"server operation timed out after $handleTimeout"))
        asyncContext.complete()
      }
      def onError(event: AsyncEvent): Unit = cancelable.cancel()
  ...
  private def writeHeaders(response: HttpServletResponse, restResponse: RestResponse): Unit = {
